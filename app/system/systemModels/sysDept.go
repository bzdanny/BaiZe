...
type SysDeptAdd struct {
	DeptId    int64   `json:"DeptId,string" db:"dept_id"`
	ParentId  int64   `json:"parentId,string" db:"parent_id" binding:"required"`
	DeptName  string  `json:"deptName" db:"dept_name" binding:"required"`
	Ancestors string  `json:"ancestors" db:"ancestors"`
	OrderNum  *uint32 `json:"orderNum" db:"order_num"`
	Leader    string  `json:"leader" db:"leader"`
	Phone     string  `json:"phone" db:"phone"`
	Email     string  `json:"email" db:"email"`
	Status    string  `json:"status" db:"status"`
