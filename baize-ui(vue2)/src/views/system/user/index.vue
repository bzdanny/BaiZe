...
import { resolveBlob } from '@/utils/zipExcel'
  ...
  methods: {
    ...
    getTreeselect() {
      listDept().then(response => {
        this.deptOptions = this.handleProps(response.data, 'deptId', 'deptName')
      })
    },
    // 筛选节点
    ...
    /** 提交按钮 */
    submitForm: function() {
      if (this.buttonOnOrOff) {
        this.buttonOnOrOff = false
        this.$refs['form'].validate(valid => {
          if (valid) {
            if (this.form.userId != undefined) {
              updateUser(this.form).then(response => {
                this.msgSuccess('修改成功')
                this.buttonOnOrOff = true
                this.open = false
                this.getList()
              })
            } else {
